...
import {Context} from '../context/sofacoverContext';
import Icon from 'react-native-vector-icons/Ionicons';
Icon.loadFont();
const {width, height} = Dimensions.get('window');
const SofacoverScreen = ({navigation}) => {
  const {
    state,
    increaseCounter,
    decreaseCounter,
  ...
  const Counter = ({name, display = '', price = null}) => {
    return (
      <View>
        <View
          style={{
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      <ScrollView>
        ...
        <Counter name="sofa_single" display="Sofa Cover(S)" price="150" />
        <Counter name="sofa_double" display="Sofa Cover(M)" price="280" />
        <Counter name="sofa_triple" display="Sofa Cover(L)" price="450" />
        <Counter name="sofa" display="Long L-shape" price="650" />
...
export default SofacoverScreen;
